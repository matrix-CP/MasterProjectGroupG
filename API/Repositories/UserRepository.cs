        ...
        public tblUser Loginmvc(tblUser user)
        {
            try
            {
                ...
                using (var cmd = new NpgsqlCommand("SELECT c_uid, c_uname, c_uemail, c_password, c_role FROM t_usermaster WHERE c_uemail=@uemail AND c_password=@password;", conn))
                {
                    ...
                    if (data.Rows.Count == 1)
                    {
                        Console.WriteLine($"userid: {Convert.ToInt32(data.Rows[0]["c_uid"])}");
                        Console.WriteLine($"username: {data.Rows[0]["c_uname"].ToString()}");
                        Console.WriteLine($"email: {data.Rows[0]["c_uemail"].ToString()}");
                        ...
                        if (session != null)
                        {
                            session.SetInt32("userid", user1.c_uid);
                            session.SetString("username", user1.c_uname);
                            session.SetString("email", user1.c_uemail);
                            session.SetInt32("IsAuthenticated", 1);
                        }
                    }
                }
